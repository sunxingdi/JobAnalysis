...
class Https:
    ...
    def get(self, url, headers=None,timeOut=5, timeOutRetry=5):
        '''
        获取网页源码
        url: 网页链接
        headers: headers
    ...
    def post(self,url, para, headers=None, timeOut=5, timeOutRetry=5):
        '''
        post获取响应
        url: 目标链接
        para: 参数
        ...
        try:
            response = requests.post(url, data=para, headers=headers,timeout=timeOut)
            if response.status_code == 200 or response.status_code == 302:
                htmlCode = response.text
            else:
                htmlCode = 'None'
